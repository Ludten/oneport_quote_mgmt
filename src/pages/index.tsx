...
const Index = () => {
  ...
  const changeMonth = (increment: number) => {
    const newDate = new Date(
      Date.UTC(
        ...
        currentDate.getUTCMonth() + increment,
        1,
      ),
    );
  ...
  return (
    <div className="min-h-screen bg-white px-1 lg:px-10 py-10 lg:py-14">
      <div className="flex flex-col gap-4 lg:gap-8">
        <header className="flex flex-col lg:flex-row justify-center text-center lg:justify-between lg:items-start gap-4 text-white">
          <div className="flex flex-col lg:items-start">
            <h1 className="text-2xl text-[#1F2937]  font-semibold">
              All Existing Quotes
            </h1>
          ...
          <div className="calendar-header flex justify-between items-center gap-2">
            <button
              ...
              className="gg-chevron-left order-1 lg:order-2"
            ></button>
            <h2 className="text-2xl font-semibold text-[#1F2937] order-2 lg:order-1">
              {currentDate.toLocaleString("default", { month: "long" })}{" "}
              <span className="text-[#00861E]">{year}</span>
            </h2>
            <button
              onClick={() => changeMonth(1)}
              className="gg-chevron-right order-3"
            ></button>
          </div>
        </header>
